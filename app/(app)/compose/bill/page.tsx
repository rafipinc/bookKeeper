...
export default async function ComposeBillPage() {
      ...
      <section className="space-y-4">
        ...
        <div className="bkp-card max-w-xl p-5">
          <p className="text-sm text-[var(--text-secondary)]">
            Connect Xero and run the initial sync before composing bills.
          </p>
  ...
  const [{ data: contacts }, { data: accounts }, { data: taxRates }] = await Promise.all([
    ...
    supabase
      .from("xero_accounts")
      .select("id,xero_tenant_id,code,name,class")
      .eq("platform_tenant_id", platformTenantId)
      .in("xero_tenant_id", xeroTenantIds)
      .eq("class", "EXPENSE")
  ...
  if (!readyConnections.length) {
    return (
      <section className="space-y-4">
        ...
        <div className="bkp-card max-w-xl p-5">
          <p className="text-sm text-[var(--text-secondary)]">
            We still need synced suppliers, expense accounts, and tax rates from Xero before this composer can be used.
          </p>
          <a className="bkp-button mt-4 inline-flex h-10 items-center px-4 text-sm" href="/settings/integrations">
