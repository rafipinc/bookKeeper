# CLAUDE.md — bookkeeping-app

Loaded automatically when Claude Code launches in this directory. Brief on purpose; the long-form context lives in the agent-workflow repo.
...
- **Role subagents** live in `../agent-workflow/.claude/agents/` — not loaded in this session. Switch to that directory's Claude Code to invoke Reviewer, Tech Lead, etc.
...
After opening a PR, switch to a Claude Code session in `../agent-workflow/` and say *"Use the reviewer subagent to review PR <URL>."* The Reviewer is advisory — Rafi makes the final merge call.
